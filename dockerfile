...
# Créer dossier de sortie pour binaire + libs
RUN mkdir -p /opt/xbeach_output/libs

# Copier le binaire
RUN cp /usr/local/bin/xbeach /opt/xbeach_output/

# Identifier les libs essentielles et les copier
RUN ldd /usr/local/bin/xbeach | grep "=>" | awk '{print $3}' | xargs -I {} cp {} /opt/xbeach_output/libs/ || true

# Changer droits pour exécution
RUN chmod -R +x /opt/xbeach_output
